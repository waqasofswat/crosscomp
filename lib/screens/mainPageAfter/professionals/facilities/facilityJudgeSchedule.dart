...
import 'package:cross_comp/screens/mainPageAfter/professionals/facilities/facilityRecruitScreen.dart';
import 'package:cross_comp/utilities/constants.dart';
import 'package:cross_comp/utilities/size_config.dart';
import 'package:flutter/material.dart';
import 'package:http/http.dart' as http;

class FacilityJudgeSchedule extends StatefulWidget {
  String fid;

  FacilityJudgeSchedule({Key? key, required this.fid}) : super(key: key);

  @override
  _FacilityJudgeScheduleState createState() => _FacilityJudgeScheduleState();
}

class _FacilityJudgeScheduleState extends State<FacilityJudgeSchedule> {
  ...
  void initState() {
    super.initState();
    getFaccilityJudgesData();
    // handleAppLifecycleState();
  }

  Future<Map<String, dynamic>> getFaccilityJudgesData() async {
    print("getEventParticipantsData");

    _timimg.clear();
    _part.clear();
    String url = mainApiUrl +
        "?get_event_facility_judge=true&id=${widget.fid}&type=facility";
      ...
      if (response.body.toString().contains("Failure")) {
        ...
        if (map['status'] == "failed") {
          ...
          try {
            ...
            dayDate = map['ef_response']['Day'].toString() +
                ", " +
                moonLanding.day.toString() +
                " " +
                monthArr[moonLanding.month] +
                ", " +
                moonLanding.year.toString();
            ...
            try {
              ...
              for (int i = 0; i < size; i++) {
                String currDate = dateX.day.toString() +
                    ...
                    "/" +
                    dateX.year.toString();
                String day = map['server_response'][i]['judges_appointments']
                        ['day']
                    .toString();
                for (int i = 0; i < 7; i++) {
                ...
                String xTiming = map['server_response'][i]
                        ['judges_appointments']['timing']
                    .toString();
                String tmng = day + ", " + currDate + ", " + xTiming;
                String participant =
                    map['server_response'][i]['participants'].toString();
                ...
                String name = map['server_response'][i]['judge']['First_Name']
                        .toString() +
                    " " +
                    map['server_response'][i]['judge']['Last_Name'].toString();
                int indeX = -1;
                String rid = map['server_response'][i]['judges_appointments']
                        ['id']
                    .toString();
                String uid =
                    map['server_response'][i]['judge']['User_ID'].toString();
                for (int x = 0; x < sizeT; x++) {
                  if (_timimg[x]["timing"].toString() == tmng) indeX = x;
                }

                if (indeX != -1) {
                  _part[indeX].add(
                      {"names": name, "id": rid, "uid": uid, "status": false});
                } else {
                  _timimg.add({
                    "timing": tmng,
  ...
  Widget animatedTileO(BuildContext context, int index, animation,
    ...
    return judgesRow(animation, part, textStyle);
  }

  SlideTransition judgesRow(
  ...
  Container newMethodEvent(
    Map<String, dynamic> item,
  ) {
    String timingText = item["timing"].toString();
    String participantText = item["participant"].toString();
    return Container(
                      ...
                      ElevatedButton(
                        ...
                        onPressed: () {
                          Navigator.push(
                              context,
                              MaterialPageRoute(
                                  builder: (context) => FacilityRecruit()));
                        },
                        child: Text(
                          "Recruit",
                          style: TextStyle(
