...
import 'package:cross_comp/screens/mainPageAfter/volunteer/assistantTrainers/trainee.dart';
...
class AssistantTrainerSchedule extends StatefulWidget {
  AssistantTrainerSchedule({Key? key}) : super(key: key);

  @override
  _AssistantTrainerScheduleState createState() =>
      _AssistantTrainerScheduleState();
}

class _AssistantTrainerScheduleState extends State<AssistantTrainerSchedule> {
      ...
      if (response.body.toString().contains("Failure")) {
        ...
        if (map['status'] == "failed") {
          ...
          try {
            ...
            print("total Array Size $size");
            DateTime dateX = DateTime.now();
            for (int i = 0; i < size; i++) {
              print("Array $i : ${map['ef_response'][i]}");
              ...
              String status = map['server_response'][i]["status"].toString();
        ...
        children: [
          ...
          GestureDetector(
            onTap: () {
              if (isStatus) {
                ...
                //     MaterialPageRoute(builder: (context) => Trainee()));
              }
            },
            child: Center(
              child: RichText(
          ...
          GestureDetector(
            onTap: () {
              if (isStatus) {
                Navigator.push(context,
                    MaterialPageRoute(builder: (context) => Trainee(mapList: item,)));
              }
            },
            child: Column(
              children: [
