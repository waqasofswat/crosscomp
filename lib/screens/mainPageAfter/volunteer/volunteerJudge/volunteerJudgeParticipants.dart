...
import 'package:cross_comp/screens/mainPageAfter/TestScreens/legRaiseTestScreen.dart';
import 'package:cross_comp/screens/mainPageAfter/TestScreens/pushUpsTestScreen.dart';
import 'package:cross_comp/screens/mainPageAfter/TestScreens/shuttleTestScreen.dart';
import 'package:cross_comp/screens/mainPageAfter/TestScreens/squartsTestScreen.dart';
import 'package:cross_comp/screens/mainPageAfter/sharePage.dart';
...
class VolunteerJudgeParticipants extends StatefulWidget {
  Map<String, dynamic> mapList;

  VolunteerJudgeParticipants({Key? key, required this.mapList})
      : super(key: key);

  @override
  _VolunteerJudgeParticipantsState createState() =>
      _VolunteerJudgeParticipantsState();
}

class _VolunteerJudgeParticipantsState extends State<VolunteerJudgeParticipants>
 ...
 // String date = '';
  ...
  Future<Map<String, dynamic>> getParticipants() async {
    ...
    String url = mainApiUrl +
        "?get_reservation_participants=true&ef_id=${widget.mapList["ef_id"]}&day=${widget.mapList["day"]}&date=${widget.mapList["date"]}&timing=${widget.mapList["hours"]}&type=${widget.mapList["type"]}";
        ...
        if (map['status'] == "failed") {
          ...
          try {
            ...
            for (int i = 0; i < size; i++) {
              ...
              String timing =
                  map['server_response'][i]["reservation"]["timing"].toString();
            // date =
            //     map['server_response'][i]["reservation"]["date"].toString();

              String uid =
                  map['server_response'][i]["user"]["User_ID"].toString();
              String userName =
                  map['server_response'][i]["user"]["First_Name"].toString() +
                      " " +
                      map['server_response'][i]["user"]["Last_Name"].toString();
  ...
  SlideTransition participantsRow(
    ...
    bool isSquarts=false;
    bool isLegRaise=false;
    bool isPushUps=false;
    print("before");
    try{
      print(item["map"]["shuttle"].toString());
      if(double.parse(item["map"]["shuttle"].toString())>0)
        isShuttle=true;
    }catch(exception){}
    print(isShuttle);
    try{
      if(double.parse(item["map"]["squats"].toString())>0)
        isSquarts=true;
    }catch(exception){}
    try{
      if(double.parse(item["map"]["legRise"].toString())>0)
        isLegRaise=true;
    }catch(exception){}
    try{
      if(double.parse(item["map"]["pushups"].toString())>0)
        isPushUps=true;
    }catch(exception){}
                    ...
                    if (isGreen) {
                      if (!isShuttle) {
                        Navigator.pushReplacement(
                            ...
                            MaterialPageRoute(
                                builder: (context) =>
                                    ShuttleTestScreen(isProf: false,mapList: item,mapListOld: widget.mapList,)));
                      } else if (!isSquarts) {
                        Navigator.pushReplacement(
                            context,
                            MaterialPageRoute(
                                builder: (context) =>
                                    SquartsTestScreen(isProf: false,mapList: item,mapListOld: widget.mapList,)));
                      } else if (!isLegRaise) {
                        Navigator.pushReplacement(
                            context,
                            MaterialPageRoute(
                                builder: (context) =>
                                    LegRaiseTestScreen(isProf: false,mapList: item,mapListOld: widget.mapList,)));
                      } else if (!isPushUps) {
                        Navigator.pushReplacement(
                            context,
                            MaterialPageRoute(
                                builder: (context) =>
                                    PushUpsTestScreen(isProf: false,mapList: item,mapListOld: widget.mapList,)));
                      } else {

                        Navigator.push(
                            context,
                            MaterialPageRoute(
                                builder: (context) => SharePage(map: item["map"])));
                      }
                    } else {
                    ...
                    showDialog(
                      ...
                      builder: (_) => ConformDialog(
                          pressYes: () {
                            setState(() {
                              isLoading=true;
                              deleteParticipant( item["id"].toString());
                        ...
                        children: [
                          ...
                          GestureDetector(
                            onTap: () {
                              ...
                              //     context,
                              //     MaterialPageRoute(
                              //         builder: (context) => EventParticipantSchedule()));
                            },
                            child: Column(
                              children: [
                                Text(
